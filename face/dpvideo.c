...
HANDLE VideoEncStart(DWORD format, DWORD width, DWORD height, DWORD qu)
{
	...
	encinfo.m_enctype = format;// ENCODE_MP4;
...
printf("----------------------------------------IOCTL_ENCODE_START1\n");
	if(encinfo.m_encw != sifSize.Width)
	{
		horRatio = (sifSize.Width * 256 + encinfo.m_encw / 2) / encinfo.m_encw;
...
printf("----------------------------------------IOCTL_ENCODE_START2\n");
	//����ʵʱ����
	encinfo.property = ENCODE_REALTIME;
	if(ioctl((int)hHandle, IOCTL_ENCODE_SET, &encinfo) < 0)
	{
		DBGMSG(DPERROR, "%s(%d): $$$$$$$$$ IOCTL_ENCODE_SET ENCODE_REALTIME fail $$$$$$$$$\n", __FILE__, __LINE__);
		close((int)hHandle);
		return	INVALID_HANDLE_VALUE;
	}
...
DWORD VideoEncRead(HANDLE hHandle, BYTE* pData, DWORD len, DWORD* property)
{
	...
	vinfo.rbuf.timeout = 80;
	//vinfo.rbuf.property = *property;//����H264ʱ����Ҫ�����������I֡ ENCODE_IFRAME
