...
public :
	void MessageDigest(const unsigned char *szInput, unsigned int inputLen,
			unsigned char* szOutput, int iIteration = 1);
	BOOL MessageDigest(char *szInput, unsigned char* szOutput);
private :
	MD5_CTX context;
 ...
 */
void TMD5::Final(unsigned char* digest, MD5_CTX *context)
{
...
//---------------------------------------------------------------------------
/* MD5 message-digest */
void TMD5::MessageDigest(const unsigned char *szInput, unsigned int inputLen, unsigned char* szOutput, int iIteration)
{
	Init(&context);
...
BOOL TMD5::MessageDigest(char *szInput, unsigned char szOutput[])
{
	FILE* fd;
	unsigned char* pbuf;
	int flen;
	...
	Init(&context);
	pbuf = (unsigned char *)malloc(0x10000);
	while(flen != 0)
	{
		if(flen > 0x10000)
			onetime = 0x10000;
			...
			onetime = flen;
		fread(pbuf, 1, onetime, fd);
		Update(&context, pbuf, onetime);
		flen -= onetime;
	}
	free(pbuf);
	fclose(fd);
...
BOOL CalStrMd5(char* output, unsigned char* input, int len)
{
	...
	pMD5->MessageDigest(input, len, coutput);
	delete pMD5;
	for(i = 0; i < 16; i++)
		sprintf(output + 2*i, "%02x", coutput[i]);
	output[32] = 0;
	return TRUE;
}
...
BOOL CalFileMd5(char* output, char* filename)
{
	...
	ret = pMD5->MessageDigest(filename, coutput);
	delete pMD5;
	if(ret)
	{
		for(i = 0; i < 16; i++)
